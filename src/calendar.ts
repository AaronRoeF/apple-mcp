...
const log = (...args: unknown[]) => console.error('[apple-calendar]', ...args);

function resolveCalendarDB(): string {
  ...
  throw new Error(
    'Apple Calendar database not found. Checked:\n' +
...
export function handleCalendarTool(name: string, args: Record<string, unknown>): { content: Array<{ type: string; text: string }>; isError?: boolean } | null {
  try {
    switch (name) {
...
function handleRange(args: Record<string, unknown>) {
  const startDate = args.start_date as string;
  const endDate = args.end_date as string;
  if (!startDate) return err('Missing required parameter: start_date');
  if (!endDate) return err('Missing required parameter: end_date');

  const startUnix = new Date(startDate).getTime() / 1000;
  const endUnix = new Date(endDate).getTime() / 1000;

  if (isNaN(startUnix)) return err('Invalid start_date — use ISO 8601 format (e.g. 2025-03-01)');
  if (isNaN(endUnix)) return err('Invalid end_date — use ISO 8601 format (e.g. 2025-03-31)');
...
function handleUpcoming(args: Record<string, unknown>) {
